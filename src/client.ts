...
import {
  ...
  MessageItem,
  RevealOutputChannelOn, ServerOptions,
  State,
  TextDocument,
  TransportKind,
  Uri,
...
export class Validator {
  ...
  public check(textDocument: { uri: string, languageId: string }): Validate {
    const config = Workspace.getConfiguration('eslint', Uri.parse(textDocument.uri));

    if (!config.get<boolean>('enable', true)) {
      return Validate.off;
    }

    if (Uri.parse(textDocument.uri).scheme === 'untitled' && config.get<boolean>('ignoreUntitled', false)) {
      return Validate.off;
    }
...
export namespace ESLintClient {
  ...
  export async function migrateSettings(client: LanguageClient): Promise<void> {
    ...
    if (folders === undefined) {
      void Window.showErrorMessage('ESLint settings can only be converted if coc.nvim is opened on a workspace folder.');
      return;
    }
    ...
    const migration = new Migration(Uri.parse(folder.uri));
    migration.record();
  ...
  export function create(context: ExtensionContext, validator: Validator): [LanguageClient, () => void] {
    ...
    const languageStatus = Window.createStatusBarItem();
    ...
    client.onRequest(ProbeFailedRequest.type, (params) => {
      ...
      for (const document of Workspace.textDocuments) {
        if (document.uri.toString() === uri.toString()) {
          ...
          let provider = diagnosticsFeature?.getProvider(document)
      ...
      const clientOptions: LanguageClientOptions = {
        ...
        middleware: {
          ...
          provideCodeActions: async (document, range, context, token, next): Promise<(Command | CodeAction)[] | null | undefined> => {
            ...
            const only = Array.isArray(context.only) ? context.only[0] : undefined
              ...
              if (migration.needsUpdate()) {
                ...
                if (selected !== undefined) {
                  if (selected.id === 'yes') {
                    ...
                    notNow = true;
                    void Workspace.openResource('https://github.com/microsoft/vscode-eslint#settings-migration');
                  }
                }
              }
            } finally {
